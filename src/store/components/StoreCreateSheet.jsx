...
export function StoreCreateSheet({
  ...
  const usaOfertas = isModuleActive(tiendaData, 'ofertas');
  ...
  if (usaOfertas) {
    opts.push({
      ...
      title: 'Nueva oferta',
      desc: atProductLimit
        ? `Límite alcanzado: ${productLimit} ofertas (${isEmprendimiento ? 'upgrade a Empresa' : 'upgrade a Premium'})`
        : 'Publicá una oferta con foto',
      locked: atProductLimit,
      action: abrirOferta,
    });
  }
      ...
      <div className="relative bg-surface-card rounded-t-3xl px-4 pt-3 shadow-2xl" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 6rem)', animation: createSheetClosing ? 'sheet-down .22s ease forwards' : 'sheet-up .22s ease' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-2 pb-2">
          {opts.map(opt => {
            const Icon = opt.icon;
            return (
              <button key={opt.key} onClick={opt.locked ? undefined : opt.action} disabled={opt.locked}
                className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-colors ${opt.locked ? 'border-slate-100 dark:border-white/8 bg-surface-card-2 dark:bg-white/5 opacity-50 cursor-not-allowed' : 'border-slate-100 dark:border-white/8 bg-surface-card-2/60 dark:bg-white/[0.04] hover:border-primary hover:bg-primary/5'}`}>
