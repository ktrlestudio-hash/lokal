    ...
    <div ref={ref} className="relative flex-1">
      <button
        ...
        className={`w-full flex items-center gap-2 text-left transition-colors focus:outline-none ${
          compact
            ? 'px-3 py-1.5 rounded-full border border-slate-200 dark:border-white/10 bg-surface-card-2 dark:bg-white/8 hover:border-primary/40'
            : 'px-3 py-2.5 rounded-2xl border-2 border-slate-200 dark:border-white/10 bg-surface-card hover:border-emerald-400 dark:hover:border-emerald-500'
        }`}
      >
      ...
      {open && (
        <div className="absolute z-50 left-0 right-0 bottom-full mb-1.5 rounded-2xl border-2 border-slate-200 dark:border-white/10 bg-surface-card shadow-xl overflow-hidden animate-dropdown-in" style={{ maxHeight: 220, overflowY: 'auto' }}>
          {options.map(o => (
            <button
              key={o.value}
              type="button"
              ...
              className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-2 transition-colors
                ${o.value === value
                  ...
                  : 'text-ink hover:bg-surface-card-2 dark:hover:bg-white/5'
                }`}
            >
