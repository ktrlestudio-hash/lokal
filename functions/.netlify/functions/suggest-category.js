// suggest-category.js — Cloudflare Pages Functions version.
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!titulo) return jsonResponse(event, 400, { error: 'titulo requerido' }, { ...HTTP_OPTIONS, env });

    const GROQ_API_KEY = env.GROQ_API_KEY;
    if (!GROQ_API_KEY) return jsonResponse(event, 500, { error: 'GROQ_API_KEY no configurada' }, { ...HTTP_OPTIONS, env });
...
- Si la confianza es menor a 0.5, usá "otros" como categoryId`;
    ...
    if (!groqRes.ok) {
      ...
      return jsonResponse(event, 502, { error: 'Error Groq', detail: err }, { ...HTTP_OPTIONS, env });
    }

    const groqData = await groqRes.json();
    const content = groqData.choices?.[0]?.message?.content;
    if (!content) return jsonResponse(event, 502, { error: 'Respuesta vacía de Groq' }, { ...HTTP_OPTIONS, env });
