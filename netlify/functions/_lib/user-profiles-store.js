...
const LOCAL_FILE = join('/tmp', 'lokal-user-profiles.json');
const DATA_KEY = 'data/user-profiles.json';
...
export async function writeUserProfiles(data) {
  if (isR2Configured()) {
...
export function findUserProfileByUid(profiles, uid) {
  return profiles.find((item) => item.uid === uid) || null;
}
